"""
============================================================================
CG TEST -- PIPELINE CORRECTNESS VALIDATION  (not the Section 4.3 power
analysis; see cg_power_analysis.py for that)
============================================================================
This script does NOT use the real ECB SPF / Eurostat data and does NOT
produce any number quoted in the thesis text. Its purpose is different and
narrower: it PROVES that the estimation pipeline in cg_pipeline.py is
implemented correctly by recovering KNOWN, pre-specified parameters from
simulated SPF-like data generated under a sticky-information process with a
known true lambda (and hence a known true beta = lambda / (1 - lambda)).
...
It runs four checks:
  ...
  (2) MC validation ......... realistic disagreement + ~150 obs: the MEAN of
                              ...
                              (and illustrates, with generic simulation
                              parameters, how noisy a short quarterly sample
                              is in general -- for the power analysis
                              calibrated to the actual estimation sample,
                              see cg_power_analysis.py instead)
...
plot, so you see exactly what the output looks like on synthetic data.

Requires: python>=3.10, numpy, pandas, matplotlib, and cg_pipeline.py
============================================================================
"""
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    print("       illustrates general small-sample noise with generic")
    print("       parameters (lambda=0.5, ~150-400 obs); NOT calibrated to")
    print("       the real estimation sample -- see cg_power_analysis.py.")
    ...
    # ---- (3b) illustrative SHORT post-window (generic params, NOT calibrated) --
    print(line); print("(3b) SHORT POST-WINDOW  --  same true change, but only ~5 yrs of post data")
    d_short = []
    for k in range(200):
        micro, realized, labels, _ = simulate_spf(
    ...
    print("       identified (wide CI, transition dynamics dominate). This confirms")
    print("       the same qualitative conclusion as cg_power_analysis.py, with")
    print("       different, generic (not data-calibrated) simulation parameters.")
    ...
    plt.savefig("cg_scatter.png", dpi=130)
